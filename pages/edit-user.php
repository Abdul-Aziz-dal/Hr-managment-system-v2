        ...
        <div id="content">
                ...
                <div class="row mb-2">
                    <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
                        <div class="card bg-white shadow py-2 p-2">
                            <h6 class="text-gray-800"><i class="fa fa-edit"></i> Edit User</h6>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card bg-white shadow py-2 p-2">
                            <form id="form" action="" method="POST">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="UserId" id="UserId" class="form-control form-control-sm" />
                                </div>
            ...
            <script>
                $(document).ready(function() {
                    ...
                    function getUser() {
                        $.ajax({
                            url: `http://localhost/hr-onboarding-system/src/api/dashboard/get-user.php`,
                            type: 'POST',
                            data: {
                                UserId: id
                            },
                            success: function(response) {
                                if (response.status == true) {
                                    ...
                                    $("select[name='RoleId']").val(user.RoleId); // Set RoleId
                                    $("select[name='Status']").val(user.Status); // Set Status
                                } else {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status == true) {
                                    ...
                                    const selectedRole = $("select[name='RoleId']").val();
                                    if (selectedRole) {
                    ...
                    $("#form").validate({
                        ...
                        submitHandler: function(form) {
                            $.ajax({
                                url: 'http://localhost/hr-onboarding-system/src/api/dashboard/edit-user.php',
                                type: 'POST',
                                data: $(form).serialize(),
                                success: function(response) {
                                    if (response.status == true) {
